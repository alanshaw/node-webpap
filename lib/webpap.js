...
function Shoot (url, opts, cb) {
  ...
  tmp.file(function (err, msgFilePath, fd) {
    ...
    var phantomPath = opts.phantomPath || require("phantomjs").path
      , phantomProcess = childProcess.spawn(phantomPath, phantomArgs)
...
function initMsgPoll (shoot) {
  ...
  function cleanup () {
    ...
    fs.unlink(shoot.msgFilePath, function (er) {
      shoot.emit("halt", er)
    })
  }
...
Shoot.prototype.halt = function (cb) {
  this.halted = true
  if (cb) this.once("halt", cb)
}
